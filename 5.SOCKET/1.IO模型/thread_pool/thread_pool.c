...
/*void delete(int signum) {
    struct ChatMsg tmp1;
    if (recv(sockfd, &tmp1, sizeof(tmp1), 0) <= 0) return ;
    DBG(GREEN"<After recv>" NONE " : tmp1.type & CHAT_FIN = %d\n", tmp1.type & CHAT_FIN);
    if (tmp1.type & CHAT_FIN) {
        DBG(GREEN"<In Delete>" NONE " : tmp1.type & CHAT_FIN1 = %d\n", tmp1.type & CHAT_FIN1);
        tmp1.type |= CHAT_FIN1;
        send(user[sockfd].fd, &tmp1, sizeof(tmp1), 0);
    }
}*/

void do_work(int fd) {
    struct ChatMsg tmp;
    if (recv(fd, tmp.msg, sizeof(tmp.msg), 0) <= 0) {
        epoll_ctl(epollfd, EPOLL_CTL_DEL, fd, NULL);
        DBG(RED"<Reactor>"NONE" : Del from reactor!\n");
        close(fd);
        return ;
    } 
    printf("<%s> : msg = %s\n", user[fd].real_name, tmp.msg);
}
